...
struct eh_frame_hdr
{
    ...
    dwfenc eh_frame_pointer_encoding; // Exception Handler Frame Pointer
                                      // Encoding
    dwfenc eh_frame_desc_entry_count_encoding; // Encoding of # of
                                               // Exception Handler FDEs
...
// WARNING! conflicting data type names: /DWARF/libio.h/_IO_marker -
// /libio.h/_IO_marker

// WARNING! conflicting data type names: /DWARF/libio.h/_IO_FILE -
// /stdio.h/_IO_FILE
...
typedef enum anon_enum_32
    .conflict1d9{ALL_PROCESSES_IN_EXIT = -606,
                 ...
                 LOGTO_BOTH_CODE = 7,
                 NON_PREEMPTIVE_CODE = 8,
                 PREEMPTIVE_CODE = 9} anon_enum_32.conflict1d9;
...
// WARNING! conflicting data type names: /time.h/timeval -
// /DWARF/time.h/timeval
...
// WARNING! conflicting data type names: /stdio.h/FILE -
// /DWARF/stdio.h/FILE
...
struct Elf64_Rela
{
    ...
    qword r_addend; // a constant addend used to compute the relocatable
                    // field value
};
...

{
    ...
    __libc_start_main(main, in_stack_00000000, &stack0x00000008,
                      __libc_csu_init, __libc_csu_fini, param_3, auStack8);
    do
    {
...

{
    ...
    {
        _Var1 = getConfigData(cmdLineData.fileName, &configDataPtr,
                              errorMessage);
        if (_Var1 == false)
        {
        ...
        if ((configUploadSuccess != false) &&
            ((cmdLineData.mdDisplayFlag != false ||
              (cmdLineData.runSimFlag != false))))
        {
            _Var1 = getMetaData(configDataPtr->metaDataFileName,
        ...
        for (; argIndex < numArgs; argIndex = argIndex + 1)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        iVar4 =
                            getStringLength(strVector[(long)numArgs + -1]);
                        iVar5 = findSubString(
                            strVector[(long)numArgs + -1], ".cnf");
    ...
    else
    {
        for (; wkgPtr != (PcbNodeType *)0x0; wkgPtr = wkgPtr->nextNode)
        {
            printf("PCB pid: %d, state: %d, time remaining: %d,\n",
                   (ulong)(uint)wkgPtr->pid, (ulong)(uint)wkgPtr->state,
                   (ulong)(uint)wkgPtr->timeRemaining);
...

{
    ...
    else if (config->cpuSchedCode == 3)
    {
        ...
        if (((pPVar2->state != 3) && (pPVar2->state != 4)) &&
    ...
    {
        for (; (wkgPtr != (PcbNodeType *)0x0 && (wkgPtr->state != 1));
...

{
    ...
    if ((pcbListPtr == (PcbNodeType *)0x0) ||
        (wkgPtr = pcbListPtr,
         newNode->opCodeList->intArg2 < pcbListPtr->opCodeList->intArg2))
    {
    ...
    else
    {
        while ((wkgPtr->nextNode != (PcbNodeType *)0x0 &&
                (wkgPtr->nextNode->opCodeList->intArg2 <=
                 newNode->opCodeList->intArg2)))
        {
            wkgPtr = wkgPtr->nextNode;
        }
...
_Bool processIntRequest(InterruptMgrCode ctrlCode, OpCodeType *wkgOpCode,
...

{
    ...
    else if (ctrlCode == CHECK_FOR_INTERRUPT)
    {
        ...
        while (wkgOpListPtr != (OpCodeType *)0x0)
        {
            ...
            {
                processIntRequest::intOpListHeadPtr = removeOpCodeNode(
                    processIntRequest::intOpListHeadPtr, rmvdOpCode);
                rmvdOpCode = (OpCodeType *)0x0;
                wkgOpListPtr = processIntRequest::intOpListHeadPtr;
            }
        }
    ...
    else if (ctrlCode == SET_IO_START)
    {
        ...
        processIntRequest::intOpListHeadPtr = addInterruptOpCode(
            processIntRequest::intOpListHeadPtr, wkgOpCode);
    }
            ...
            if (wkgOpListPtr->opEndTime < dVar2)
            {
                if (pPVar1->state != 4)
                {
                    sprintf(reportString,
                            "OS: Interrupted by process %d, %s %sput "
                            "operation",
                            (ulong)(uint)wkgOpListPtr->pid,
                            wkgOpListPtr->strArg1, wkgOpListPtr->inOutArg);
                    runSimReport(1, (ConfigDataType *)0x0, reportString);
                    pPVar1->state = 1;
                    sprintf(reportString,
                            "OS: Process %d set from BLOCKED to READY",
                            (ulong)(uint)wkgOpListPtr->pid);
                    runSimReport(1, (ConfigDataType *)0x0, reportString);
                }
                rmvdOpCode = wkgOpListPtr;
...

{
    ...
    processIntRequest(INITIALIZE_INTERRUPT_MGR, (OpCodeType *)0x0,
                      pcbListPtr_00);
    ...
    do
    {
        ...
        else
        {
            ...
            if ((((iVar4 != 0) ||
                  (iVar4 = compareString(wkgOpCode->strArg1, "start"),
                   iVar4 != 0)) &&
                 (iVar1 == 9)) &&
                (pcbIdNum != lastPid))
            {
                ...
                sprintf(reportString,
                        "OS: Process %d set from READY to RUNNING",
                        (ulong)pcbIdNum);
                runSimReport(1, configPtr, reportString);
            }
            iVar4 = compareString(wkgOpCode->command, "app");
            ...
            else
            {
                iVar4 = compareString(wkgOpCode->command, "app");
                if ((iVar4 == 0) &&
                    (iVar4 = compareString(wkgOpCode->strArg1, "end"),
                     iVar4 == 0))
                {
                ...
                else
                {
                    ...
                    if (iVar4 == 0)
                    {
                        ...
                        if (_Var3 == false)
                        {
                            ...
                            sprintf(
                                reportString,
                                "OS: Segmentation fault, Process %d ended",
                                (ulong)(uint)wkgOpCode->pid);
                            runSimReport(1, configPtr, reportString);
                    ...
                    else
                    {
                        ...
                        if (iVar4 == 0)
                        {
                            sprintf(
                                ...
                                (ulong)(uint)wkgOpCode->pid,
                                wkgOpCode->strArg1, wkgOpCode->inOutArg);
                            runSimReport(1, configPtr, reportString);
                            wkgOpCode->intArg2 = wkgOpCode->intArg2 *
                                                 configPtr->ioCycleRate;
                            processIoOp(wkgOpCode, pcbListPtr_00);
                            pcbWkgPtr =
                                getPcbById(pcbListPtr_00, pcbIdNum);
                            pcbWkgPtr->timeRemaining =
                                pcbWkgPtr->timeRemaining -
                                wkgOpCode->intArg2;
                            wkgOpCode->intArg2 = 0;
                        }
                        ...
                        {
                            iVar4 =
                                compareString(wkgOpCode->command, "cpu");
                            if (iVar4 == 0)
                            {
                                sprintf(
                                    reportString,
                                    ...
                                    wkgOpCode->strArg1);
                                runSimReport(1, configPtr, reportString);
                                processRunOp(configPtr, wkgOpCode,
                                             pcbListPtr_00);
                            }
                        }
                    }
        ...
        processIntRequest(RESOLVE_ALL_INTERRUPTS, wkgOpCode,
                          pcbListPtr_00);
        lastPid = pcbIdNum;
    } while (bVar2);
...
void runSimReport(int reportControl, ConfigDataType *configPtr,
                  char *textString)
...
LAB_00401ebc:
    ...
    if (local_cc == 1)
    {
        ...
        if ((runSimReport::config->logToCode == 7) ||
            (runSimReport::config->logToCode == 6))
        {
            runSimReport::localNode =
                addSimNode(runSimReport::localNode, outputString);
        }
        if ((runSimReport::config->logToCode == 7) ||
            (runSimReport::config->logToCode == 5))
        {
            puts(outputString);
        }
    }
    else
    {
        ...
        configCodeToString(runSimReport::config->cpuSchedCode,
                           outputString);
        ...
        for (; workingNode != (ReportNode *)0x0;
        ...
        fclose(__s);
        runSimReport::localNode =
            removeReportNodes(runSimReport::localNode);
        free(runSimReport::config);
    }
    return;
}

PcbNodeType *setAllPCB(ConfigDataType *configPtr,
...

{
    ...
    if ((iVar1 == 0) &&
        (iVar1 = compareString(metaDataMstrPtr->strArg1, "start"),
         iVar1 == 0))
    {
        local_48 = metaDataMstrPtr->nextNode;
        while ((iVar1 = compareString(local_48->command, "sys"),
                iVar1 != 0 &&
                    (iVar1 = compareString(local_48->strArg1, "end"),
                     iVar1 != 0)))
        {
            iVar1 = compareString(local_48->command, "app");
            if ((iVar1 == 0) &&
                (iVar1 = compareString(local_48->strArg1, "start"),
                 iVar1 == 0))
            {
                ...
                while ((iVar1 = compareString(local_48->command, "app"),
                        iVar1 != 0 || (iVar1 = compareString(
                                           local_48->strArg1, "end"),
                                       iVar1 != 0)))
                {
                    ...
                    if (iVar1 != 0)
                    {
                        ...
                        if ((configPtr->cpuSchedCode == 0) ||
                            (configPtr->cpuSchedCode == 4))
                        {
                            local_48->intArg3 = 8;
                        }
                        else
...

{
    ...
    for (wkgPtr = pcbListPtr; wkgPtr != (PcbNodeType *)0x0;
         wkgPtr = wkgPtr->nextNode)
    {
        if (wkgPtr->state == 0)
        {
...
MemNodeType *createNewMemNode(int physStart, int physEnd, int memState,
                              int procNum, int logStart, int logStop)
...

{
    ...
    {
        if (((wkgPtr->memBlockState == 1) &&
             (wkgPtr->nextNode != (MemNodeType *)0x0)) &&
            (wkgPtr->nextNode->memBlockState == 1))
        {
            reuseMemNode(wkgPtr, 1, -1, wkgPtr->physicalStart,
                         wkgPtr->nextNode->physicalStop, 0, 0);
...

{
    ...
    if (displayFlag != false)
    {
             ...
             wkgMemPtr = wkgMemPtr->nextNode)
        {
            ...
            {
                printf("%d [ %s, P#: x, ",
                       (ulong)(uint)wkgMemPtr->physicalStart, dispString);
            }
            else
            {
                printf("%d [ %s, P#: %d, ",
            ...
            printf("%d-%d ] %d\n", (ulong)(uint)wkgMemPtr->logicalStart,
                   (ulong)(uint)wkgMemPtr->logicalStop,
...

{
    ...
    {
        processMemRequest::memHeadPtr =
            createNewMemNode(0, configPtr->memAvailable + -1, 1, -1, 0, 0);
    ...
    else
    {
        ...
        if (((iVar4 == 0) ||
            ...
            ((logicalLow < 0 ||
              (((logicalOffset < 1 ||
                 (configPtr->memAvailable <= logicalLow)) ||
        ...
        else
        {
            ...
            if (iVar4 == 0)
            {
                for (wkgMemPtr = processMemRequest::memHeadPtr;
                     wkgMemPtr != (MemNodeType *)0x0;
                     wkgMemPtr = wkgMemPtr->nextNode)
                {
                    if ((wkgMemPtr->processNumber == procNum) &&
                        (((wkgMemPtr->logicalStart <= logicalLow &&
                           (logicalLow <= wkgMemPtr->logicalStop)) ||
                          ((wkgMemPtr->logicalStart <= iVar3 &&
                            (iVar3 <= wkgMemPtr->logicalStop))))))
                    {
                        copyString(displayStr,
                                   "After allocate overlap failure");
                        diagMemDump(processMemRequest::memHeadPtr,
                                    displayStr, _Var2);
                        return false;
                    }
                }
                ...
                {
                    if ((wkgMemPtr->physicalStop -
                         wkgMemPtr->physicalStart) +
                            1 !=
                        logicalOffset)
                    {
                        iVar4 = logicalOffset + wkgMemPtr->physicalStart;
                        pMVar5 = createNewMemNode(wkgMemPtr->physicalStart,
                                                  iVar4 + -1, 2, procNum,
                                                  logicalLow, iVar3);
                        pMVar1 = pMVar5;
                    ...
                    reuseMemNode(
                        wkgMemPtr, 2, procNum, wkgMemPtr->physicalStart,
                        wkgMemPtr->physicalStop, logicalLow, iVar3);
                }
                copyString(displayStr, "After allocate failure");
                diagMemDump(processMemRequest::memHeadPtr, displayStr,
                            _Var2);
            }
            else
            {
                ...
                if (iVar4 != 0)
                {
                    ...
                    if (iVar3 == 0)
                    {
                             ...
                             wkgMemPtr = wkgMemPtr->nextNode)
                        {
                            if (wkgMemPtr->processNumber == procNum)
                            {
                                reuseMemNode(wkgMemPtr, 1, -1,
                                             wkgMemPtr->physicalStart,
                                             wkgMemPtr->physicalStop, 0,
                                             0);
                            }
                        }
...
void reuseMemNode(MemNodeType *tempNode, int memState, int procNum,
...

{
    ...
    if (opCodePtr->intArg3 == 8)
    {
        ...
        sprintf(reportString, "Process: %d, %s %sput operation end",
                (ulong)(uint)opCodePtr->pid, opCodePtr->strArg1,
                opCodePtr->inOutArg);
        runSimReport(1, (ConfigDataType *)0x0, reportString);
    }
    else
    {
        sprintf(reportString, "OS: Process %d blocked for %sput operation",
                (ulong)(uint)opCodePtr->pid, opCodePtr->inOutArg);
        runSimReport(1, (ConfigDataType *)0x0, reportString);
...
void processRunOp(ConfigDataType *configPtr, OpCodeType *opCodePtr,
                  PcbNodeType *pcbListPtr)

{
    ...
    while (bVar1)
    {
        ...
        interruptFound =
            processIntRequest(CHECK_FOR_INTERRUPT, opCodePtr, pcbListPtr);
        if ((cyclesToRun == 0) || (((bVar3 && (quantumCount == 0)) ||
                                    (interruptFound != false))))
        {
            bVar1 = false;
        }
    ...
    if (((cyclesToRun < 1) || (!bVar3)) || (interruptFound == true))
    {
        sprintf(reportString, "Process: %d, %s %s operation end",
                (ulong)(uint)opCodePtr->pid, opCodePtr->command,
                opCodePtr->strArg1);
        runSimReport(1, configPtr, reportString);
    }
    else
...

{
    ...
    {
        runTimer(milliSeconds);
        interruptFound = processIntRequest(CHECK_FOR_INTERRUPT,
                                           (OpCodeType *)0x0, pcbListPtr);
    }
    reportString._0_8_ = 0x20555043203a534f;
...

{
    ...
    puts("----------------------\n");
    for (local_10 = localPtr; local_10 != (OpCodeType *)0x0;
         local_10 = local_10->nextNode)
    {
        printf("Op Code: ");
...
_Bool getMetaData(char *fileName, OpCodeType **opCodeDataHead,
                  char *endStateMsg)
             ...
             iVar2 == 0))
        {
            ...
            if (accessResult == COMPLETE_OPCMD_FOUND_MSG)
            {
                while (accessResult == 7)
                {
                    ...
                    accessResult = getOpCommand((FILE *)inStream, inData);
                    startCount =
                        updateStartCount(startCount, inData->strArg1);
                ...
                if (accessResult == 8)
                {
                    if (startCount != endCount)
                    {
                        ...
                        *opCodeDataHead = pOVar3;
                        free(inData);
                        copyString(endStateMsg, "Unbalanced start and end "
                                                "arguments in metadata");
                        return false;
                    }
                    localHeadPtr = addNode(localHeadPtr, inData);
                    accessResult = 0;
                    _Var1 = getStringToDelimiter((FILE *)inStream, '.',
                                                 dataBuffer);
                    if ((_Var1 != true) ||
                        (iVar2 = compareString(
                             dataBuffer, "End Program Meta-Data Code"),
                         iVar2 != 0))
                    {
                        accessResult = 2;
...

{
    ...
    runningStringIndex =
        getCommand(cmdBuffer, strBuffer, runningStringIndex);
    ...
    if (iVar2 == 0)
    {
        ...
        if ((iVar2 != 0) &&
            (iVar2 = compareString(argStrBuffer, "out"), iVar2 != 0))
        {
    ...
    if ((iVar2 == 0) &&
        (iVar2 = compareString(local_a8->strArg1, "end"), iVar2 == 0))
    {
        return LAST_OPCMD_FOUND_MSG;
    }
    iVar2 = compareString(local_a8->command, "app");
    if ((iVar2 == 0) &&
        (iVar2 = compareString(local_a8->strArg1, "start"), iVar2 == 0))
    {
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (iVar2 == 0)
                {
                    runningStringIndex = getNumberArg(
                        &numBuffer, strBuffer, runningStringIndex);
                    if (numBuffer < 0)
                    {
                        arg2FailureFlag = true;
                    }
                    local_a8->intArg2 = numBuffer;
                    getNumberArg(&numBuffer, strBuffer,
                                 runningStringIndex);
                    if (numBuffer < 0)
                    {
...

{
    ...
    *number = 0;
    multiplier = 1;
    for (local_2c = index;
         (inputStr[local_2c] < '!' || (inputStr[local_2c] == ','));
         local_2c = local_2c + 1)
    {
    }
    for (; (_Var2 = isDigit(inputStr[local_2c]),
           _Var2 != false && (inputStr[local_2c] != '\0'));
         local_2c = local_2c + 1)
    {
        bVar1 = true;
...

{
    ...
    if (((((iVar1 != 0) &&
           (iVar1 = compareString(strArg, "allocate"), iVar1 != 0)) &&
          (iVar1 = compareString(strArg, "end"), iVar1 != 0)) &&
         ((((iVar1 = compareString(strArg, "ethernet"),
             iVar1 != 0 && (iVar1 = compareString(strArg, "hard drive"),
                            iVar1 != 0)) &&
            ((iVar1 = compareString(strArg, "keyboard"),
              iVar1 != 0 && ((iVar1 = compareString(strArg, "monitor"),
                              iVar1 != 0 && (iVar1 = compareString(
                                                 strArg, "printer"),
                                             iVar1 != 0)))))) &&
           (iVar1 = compareString(strArg, "process"), iVar1 != 0)))) &&
        ((((iVar1 = compareString(strArg, "serial"),
            iVar1 != 0 && (iVar1 = compareString(strArg, "sound signal"),
                           iVar1 != 0)) &&
           (iVar1 = compareString(strArg, "start"), iVar1 != 0)) &&
          ((iVar1 = compareString(strArg, "usb"),
            iVar1 != 0 && (iVar1 = compareString(strArg, "video signal"),
                           iVar1 != 0))))))
    {
        return false;
    }
...

{
    ...
    if ((((iVar1 != 0) &&
          (iVar1 = compareString(testCmd, "app"), iVar1 != 0)) &&
         (iVar1 = compareString(testCmd, "cpu"), iVar1 != 0)) &&
        ((iVar1 = compareString(testCmd, "mem"),
          iVar1 != 0 &&
...

{
    ...
    puts("-------------------");
    printf((char *)configData->version,
           "Version                : %3.2f\n");
...
_Bool getConfigData(char *fileName, ConfigDataType **configData,
                    char *endStateMsg)

{
    ...
    *configData = (ConfigDataType *)0x0;
    ...
    else
    {
        _Var1 = getStringToDelimiter((FILE *)inStream, ':', dataBuffer);
        if ((_Var1 == true) &&
            (iVar2 = compareString(dataBuffer,
             ...
             iVar2 == 0))
        {
            ...
            while (true)
            {
                ...
                if (lineCode == CFG_CORRUPT_PROMPT_ERR)
                    break;
                if (lineCode == CFG_VERSION_CODE)
                {
                    __isoc99_fscanf(inStream, &DAT_00406735, &doubleData);
                }
                else if ((((lineCode == CFG_MD_FILE_NAME_CODE) ||
                           (lineCode == CFG_LOG_FILE_NAME_CODE)) ||
                          (lineCode == CFG_CPU_SCHED_CODE)) ||
                         ((lineCode == CFG_LOG_TO_CODE ||
                           (lineCode == CFG_MEM_DISPLAY_CODE))))
                {
                ...
                _Var1 = valueInRange(lineCode, intData, doubleData,
                                     lowerCaseDataBuffer);
                if (_Var1 == false)
                {
                    free(__ptr);
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    iVar1 = compareString(dataBuffer,
                    ...
                    else
                    {
                        ...
                        {
                            iVar1 = compareString(
                                dataBuffer, "Processor Cycle Time (msec)");
                            if (iVar1 == 0)
                            {
                            ...
                            {
                                iVar1 = compareString(
                                    dataBuffer, "I/O Cycle Time (msec)");
                                if (iVar1 == 0)
                                {
...

{
    ...
    case 6:
        ...
        if ((((iVar1 != 0) &&
              (iVar1 = compareString(lowerCaseStringVal, "sjf-n"),
               iVar1 != 0)) &&
             (iVar1 = compareString(lowerCaseStringVal, "srtf-p"),
              iVar1 != 0)) &&
            ((iVar1 = compareString(lowerCaseStringVal, "fcfs-p"),
              iVar1 != 0 &&
                  (iVar1 = compareString(lowerCaseStringVal, "rr-p"),
                   iVar1 != 0))))
        {
            result = false;
        }
    ...
    case 8:
        ...
        if ((iVar1 != 0) &&
            (iVar1 = compareString(lowerCaseStringVal, "off"), iVar1 != 0))
        {
            result = false;
        }
        break;
    ...
    case 0xc:
        ...
        if (((iVar1 != 0) &&
             (iVar1 = compareString(lowerCaseStringVal, "monitor"),
              iVar1 != 0)) &&
            (iVar1 = compareString(lowerCaseStringVal, "file"),
             iVar1 != 0))
        {
            result = false;
        }
...

{
    ...
    for (index = 0;
         ((oneStr[index] != '\0' && (otherStr[index] != '\0')) &&
          (index < 0x80));
         index = index + 1)
    {
    ...
    *capturedString = '\0';
    ...
    else
    {
        while ((((intChar != -1 && (index < 0x7f)) &&
                 (((stopAtNonPrintable != false && (0x1f < intChar)) ||
...

{
    ...
    for (index = 0; (index < 0x40 && (testStr[index] != '\0'));
         index = index + 1)
    {
    }
    return index;
...

{
    ...
    _Var1 = getStringConstrained(inStream, true, true, true, delimiter,
                                 capturedString);
    return _Var1;
}
...

{
    ...
    _Var1 = getStringConstrained(inStream, true, true, true, '\x7f',
                                 capturedString);
    return _Var1;
}
...

{
    ...
    if ((controlCode == 1) && (accessTimer::running != false))
    {
        ...
        fpTime = processTime((double)accessTimer::startSec,
                             (double)(int)lapData.tv_sec,
                             (double)accessTimer::startUSec,
                             (double)(int)lapData.tv_usec, timeStr);
    }
    else if ((controlCode == 2) && (accessTimer::running != false))
    {
        ...
        fpTime = processTime((double)accessTimer::startSec,
                             (double)(int)endData.tv_sec,
                             (double)accessTimer::startUSec,
                             (double)(int)endData.tv_usec, timeStr);
    }
    else
    {
...

{
    ...
    while (timeDiff < milliSeconds)
    {
        ...
        timeDiff =
            uSecDiff / 1000 + (endSec - (int)startTime.tv_sec) * 1000;
    }
    return;
}

void __libc_csu_init(EVP_PKEY_CTX *param_1, undefined8 param_2,
                     undefined8 param_3)
