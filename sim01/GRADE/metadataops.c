...
Function Output/Parameters: pointer to op code linked list head pointer
                            (opCodeType **),
                            result message of funcitno state after
completion (char *) Function OUtput/Returned: Boolean resuilt of operation
(bool) Device Input/device: op code list uploaded Device Output/device:
none Dependencies: copyString, fopen, getStringToDelimiter, compareString,
fclose, malloc, getOpCommand, updateStartCommand, updateEndCount,
...
Device Output/device: none
Dependencies: getSringToDelimiter. getCommand, copyString,
verifyValidCommand, compareString, getStringArg, verifyFirstSTringArg,
getNumberArg
*/
...
int getNumberArg(int *number, const char *inputString, int index)
{
  ...
  *number = 0;
  ...
  // loop across string
  while (isDigit(inputString[index]) == true && inputString[index] != NULL_CHAR)
  {
    foundDigit = true;
