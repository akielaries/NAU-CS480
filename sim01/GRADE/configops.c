...
Process: frees dynamically allocated config data structure if it has not already
         been freed
...
void configCodeToString(int code, char *outString)
{
    ...
    char displayString[8][10] = {"SJF-N", "SRTF-P", "FCFS-P", "RR-P" "FCFS-N", "Monitor", "File","BOTH"};
                                                              ...
                                                              char *endStateMsg)
{
    ...
    *configData = NULL;
                    ...
                    switch(dataLineCode)
                        {
                            ...
                            case CFG_MEM_DISPLAY_CODE:
                                tempData->memDisplay
                                  = compareString(lowerCaseDataBuffer, "on")
                                                                           == 0;
                                break;
    ...
    if (!getStringToDelimiter(fileAccessPtr, PERIOD, dataBuffer)
            || compareString(dataBuffer, 
                                "End Simolator Configuration file") != 
                                                                      STR_EQ)
    {    
        // free temp struct mem
            // function: free
