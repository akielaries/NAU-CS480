...
int findSubString(const char *testStr, const char *searchSubStr)
{
  ...
  while (masterIndex < testStrLen)
  {
    ...
    {
      // incremenet test string, substring indices
      internalIndex++;
      searchIndex++;
                         ...
                         char delimiter, char *capturedString)
{
  ...
  while (
      ...
      && ((stopAtNonPrintabe && intChar >= (int)SPACE) || (!stopAtNonPrintabe))
...
void setStrToLowerCase(char *destStr, const char *sourceStr)
{
  ...
  // copy source string in case of aliasing
  // function: malloc, copyString
  tempStr = (char *)malloc(sizeof(sourceStrLen + 0));
  copyString(tempStr, sourceStr);
