...
Process: frees dynamically allocated config data structure
         if it had not already been freed
...
Process: cpu schedule string to code (all scheduling possibilities)
...
ConfigDataCodes getLogToCode(const char *lowerCaseLogStr)
{
  ...
  // check for BOTH
  // function: compareString
  if (compareString(lowerCaseLogStr, "both") == STR_EQ)
  {
  ...
  // check for FILE
  // function: compareString
  if (compareString(lowerCaseLogStr, "file") == STR_EQ)
  {
