...
void displayConfigData( ConfigDataType *configData)
{
   ...
   // print lines of display for all member values 
      // fucntion: pirntf, codeToString(translates coded items)
                     ...
                     ConfigDataType **configData, char *endStateMsg)
{
   ...
   *configData = NULL;
   ...
   while( lineCtr < NUM_DATA_LINES)
   {
      ...
      if (dataLineCode != CFG_CORRUPT_PROMPT_ERR)
      {
         // get data value 
            ...
            // check for version number (double val)
            ...
            // CPU shceduling names, or memory display settings (strings)
            ...
            // check for data values in range 
                                               ...
                                               doubleData, lowerCaseDataBuffer) )
            {
               ...
               // (all config line possiblilites)
                  // function: copyString, getCpuSchedCode, 
                  //           compareString, getLogToCode
               switch( dataLineCode)
               {
   ...
   // test for "file only" output so memory diagnostics not display
                               ...
                               double doubleVal, const char *lowerCaseStringVal)
{
   // initialize function/variables 
      ...
      // set result to true, all tests are to see if false 
      bool result = true; 
   ...
   switch(lineCode)
   {
      ...
      case CFG_MEM_AVAILABLE_CODE: 
         ...
         // check for avalible memory limits exceeded
